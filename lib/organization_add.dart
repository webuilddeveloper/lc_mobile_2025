import 'dart:async';
import 'package:flutter/material.dart';
import 'package:lc/component/header.dart';
import 'package:lc/pages/blank_page/dialog_fail.dart';
import 'package:lc/shared/api_provider.dart';
import 'package:lc/widget/dialog.dart';

class OrganizationAddPage extends StatefulWidget {
  const OrganizationAddPage({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _OrganizationAddPageState createState() => _OrganizationAddPageState();
}

class _OrganizationAddPageState extends State<OrganizationAddPage> {
  late Future<dynamic> futureLv0;
  late Future<dynamic> futureLv1;
  late Future<dynamic> futureLv2;
  late Future<dynamic> futureLv3;
  late Future<dynamic> futureLv4;

  String lv0 = '';
  String lv1 = '';
  String lv2 = '';
  String lv3 = '';
  String lv4 = '';

  @override
  void initState() {
    _callRead();
    super.initState();
  }

  @override
  Widget build(BuildContext context) {
    return _buildScaffold();
  }

  _callRead() {
    futureLv0 =
        postDio('${server}organization/category/read', {'category': 'lv0'});
  }

  void goBack() async {
    Navigator.pop(context, false);
  }

  _buildScaffold() {
    return Scaffold(
      backgroundColor: Colors.white,
      appBar: header(
        context,
        () => {Navigator.pop(context)},
        title: 'ประเภทสมาชิก',
      ),
      body: _buildListView(),
    );
  }

  _buildListView() {
    return ListView(
      // shrinkWrap: true, // use it
      children: [
        const SizedBox(
          height: 20,
        ),
        _widgetText(title: 'การสื่อสารระดับ 1'),
        _buildLv0(),
        const SizedBox(
          height: 12,
        ),
        if (lv0 != '') _widgetText(title: 'การสื่อสารระดับ 2'),
        _buildLv1(),
        const SizedBox(
          height: 12,
        ),
        if (lv1 != '') _widgetText(title: 'การสื่อสารระดับ 3'),
        _buildLv2(),
        const SizedBox(
          height: 12,
        ),
        if (lv2 != '') _widgetText(title: 'การสื่อสารระดับ 4'),
        _buildLv3(),
        if (lv3 != '') _widgetText(title: 'การสื่อสารระดับ 5'),
        _buildLv4(),
        const SizedBox(
          height: 12,
        ),
        Container(
          margin: const EdgeInsets.only(top: 50, bottom: 50),
          padding: const EdgeInsets.only(
            left: 50,
            right: 50,
          ),
          child: TextButton(
            style: TextButton.styleFrom(
              foregroundColor: Colors.white,
              backgroundColor: const Color(0xFF707070),
              shape: RoundedRectangleBorder(
                borderRadius: BorderRadius.circular(8.0),
                side: const BorderSide(color: Color(0xFF707070)),
              ),
            ),
            onPressed: () {
              _callSave();
            },
            child: const Text('บันทึกข้อมูล'),
          ),
        )
      ],
    );
  }

  _buildLv0() {
    return FutureBuilder<dynamic>(
      future: futureLv0,
      builder: (BuildContext context, AsyncSnapshot<dynamic> snapshot) {
        if (snapshot.hasData) {
          return ListView.builder(
            shrinkWrap: true, // use it
            physics: const ClampingScrollPhysics(),
            itemCount: snapshot.data.length,
            itemBuilder: (context, index) {
              return InkWell(
                onTap: () {
                  setState(() {
                    lv1 = '';
                    lv2 = '';
                    lv3 = '';
                    lv4 = '';
                    lv0 = lv0 == snapshot.data[index]['code']
                        ? ''
                        : snapshot.data[index]['code'];

                    futureLv1 = Future.value([]);
                    futureLv2 = Future.value([]);
                    futureLv3 = Future.value([]);
                    futureLv4 = Future.value([]);

                    if (lv0 != '')
                      futureLv1 = postDio('${server}organization/category/read',
                          {'category': 'lv1', 'lv0': lv0});
                  });
                },
                child: Container(
                  margin: const EdgeInsets.only(left: 50, right: 50, bottom: 5),
                  padding: const EdgeInsets.all(5),
                  decoration: BoxDecoration(
                      borderRadius: BorderRadius.circular(10),
                      color: lv0 == snapshot.data[index]['code']
                          ? const Color(0xFF359EEA)
                          : Colors.white,
                      border: Border.all(color: const Color(0xFF359EEA))),
                  alignment: Alignment.center,
                  child: Padding(
                    padding: const EdgeInsets.fromLTRB(5, 0, 5, 0),
                    child: Row(
                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
                      children: [
                        Text(
                          '${snapshot.data[index]['title']}',
                          style: TextStyle(
                            color: lv0 == snapshot.data[index]['code']
                                ? Colors.white
                                : Colors.black,
                          ),
                        ),
                        if (lv0 == snapshot.data[index]['code'])
                          const Icon(
                            Icons.check,
                            color: Color(0xFFFFFFFF),
                          ),
                      ],
                    ),
                  ),
                ),
              );
            },
          );
        } else if (snapshot.hasError) {
          return Center(
            child: Container(
              color: Colors.white,
              child: dialogFail(context, reloadApp: true),
            ),
          );
        } else {
          return const Center(child: Text('Loading....'));
        }
      },
    );
  }

  _buildLv1() {
    return FutureBuilder<dynamic>(
      future: futureLv1,
      builder: (BuildContext context, AsyncSnapshot<dynamic> snapshot) {
        if (snapshot.hasData) {
          return ListView.builder(
            shrinkWrap: true, // use it
            physics: const ClampingScrollPhysics(),
            itemCount: snapshot.data.length,
            itemBuilder: (context, index) {
              return InkWell(
                onTap: () {
                  setState(() {
                    lv2 = '';
                    lv3 = '';
                    lv4 = '';
                    lv1 = lv1 == snapshot.data[index]['code']
                        ? ''
                        : snapshot.data[index]['code'];
                    futureLv2 = Future.value([]);
                    futureLv3 = Future.value([]);
                    futureLv4 = Future.value([]);

                    if (lv1 != '')
                      futureLv2 = postDio('${server}organization/category/read',
                          {'category': 'lv2', 'lv1': lv1});
                  });
                },
                child: Container(
                  margin: const EdgeInsets.only(left: 50, right: 50, bottom: 5),
                  padding: const EdgeInsets.all(5),
                  decoration: BoxDecoration(
                      borderRadius: BorderRadius.circular(10),
                      color: lv1 == snapshot.data[index]['code']
                          ? const Color(0xFF359EEA)
                          : Colors.white,
                      border: Border.all(color: const Color(0xFF359EEA))),
                  alignment: Alignment.center,
                  child: Padding(
                    padding: const EdgeInsets.fromLTRB(5, 0, 5, 0),
                    child: Row(
                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
                      children: [
                        Text(
                          '${snapshot.data[index]['title']}',
                          style: TextStyle(
                            color: lv1 == snapshot.data[index]['code']
                                ? Colors.white
                                : Colors.black,
                          ),
                        ),
                        if (lv1 == snapshot.data[index]['code'])
                          const Icon(
                            Icons.check,
                            color: Color(0xFFFFFFFF),
                          ),
                      ],
                    ),
                  ),
                ),
              );
            },
          );
        } else if (snapshot.hasError) {
          return Center(
            child: Container(
              color: Colors.white,
              child: dialogFail(context, reloadApp: true),
            ),
          );
        } else {
          return Center(
            child: Container(
              child: lv0 != '' ? const Text('Loading....') : Container(),
            ),
          );
        }
      },
    );
  }

  _buildLv2() {
    return FutureBuilder<dynamic>(
      future: futureLv2,
      builder: (BuildContext context, AsyncSnapshot<dynamic> snapshot) {
        if (snapshot.hasData) {
          return ListView.builder(
            shrinkWrap: true, // use it
            physics: const ClampingScrollPhysics(),
            itemCount: snapshot.data.length,
            itemBuilder: (context, index) {
              return InkWell(
                onTap: () {
                  setState(() {
                    lv3 = '';
                    lv4 = '';
                    lv2 = lv2 == snapshot.data[index]['code']
                        ? ''
                        : snapshot.data[index]['code'];

                    futureLv3 = Future.value([]);
                    futureLv4 = Future.value([]);

                    if (lv2 != '')
                      futureLv3 = postDio('${server}organization/category/read',
                          {'category': 'lv3', 'lv2': lv2});
                  });
                },
                child: Container(
                  margin: const EdgeInsets.only(left: 50, right: 50, bottom: 5),
                  padding: const EdgeInsets.all(5),
                  decoration: BoxDecoration(
                      borderRadius: BorderRadius.circular(10),
                      color: lv2 == snapshot.data[index]['code']
                          ? const Color(0xFF359EEA)
                          : Colors.white,
                      border: Border.all(color: const Color(0xFF359EEA))),
                  alignment: Alignment.center,
                  child: Padding(
                    padding: const EdgeInsets.fromLTRB(5, 0, 5, 0),
                    child: Row(
                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
                      children: [
                        Text(
                          '${snapshot.data[index]['title']}',
                          style: TextStyle(
                            color: lv2 == snapshot.data[index]['code']
                                ? Colors.white
                                : Colors.black,
                          ),
                        ),
                        if (lv2 == snapshot.data[index]['code'])
                          const Icon(
                            Icons.check,
                            color: Color(0xFFFFFFFF),
                          ),
                      ],
                    ),
                  ),
                ),
              );
            },
          );
        } else if (snapshot.hasError) {
          return Center(
            child: Container(
              color: Colors.white,
              child: dialogFail(context, reloadApp: true),
            ),
          );
        } else {
          return Center(
            child: Container(
              child: lv1 != '' ? const Text('Loading....') : Container(),
            ),
          );
        }
      },
    );
  }

  _buildLv3() {
    return FutureBuilder<dynamic>(
      future: futureLv3,
      builder: (BuildContext context, AsyncSnapshot<dynamic> snapshot) {
        if (snapshot.hasData) {
          return ListView.builder(
            shrinkWrap: true, // use it
            physics: const ClampingScrollPhysics(),
            itemCount: snapshot.data.length,
            itemBuilder: (context, index) {
              return InkWell(
                onTap: () {
                  setState(() {
                    lv4 = '';
                    lv3 = lv3 == snapshot.data[index]['code']
                        ? ''
                        : snapshot.data[index]['code'];

                    futureLv4 = Future.value([]);
                    if (lv3 != '')
                      futureLv4 = postDio('${server}organization/category/read',
                          {'category': 'lv4', 'lv3': lv3});
                  });
                },
                child: Container(
                  margin: const EdgeInsets.only(left: 50, right: 50, bottom: 5),
                  padding: const EdgeInsets.all(5),
                  decoration: BoxDecoration(
                      borderRadius: BorderRadius.circular(10),
                      color: lv3 == snapshot.data[index]['code']
                          ? const Color(0xFF359EEA)
                          : Colors.white,
                      border: Border.all(color: const Color(0xFF359EEA))),
                  alignment: Alignment.center,
                  child: Padding(
                    padding: const EdgeInsets.fromLTRB(5, 0, 5, 0),
                    child: Row(
                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
                      children: [
                        Text(
                          '${snapshot.data[index]['title']}',
                          style: TextStyle(
                            color: lv3 == snapshot.data[index]['code']
                                ? Colors.white
                                : Colors.black,
                          ),
                        ),
                        if (lv3 == snapshot.data[index]['code'])
                          const Icon(
                            Icons.check,
                            color: Color(0xFFFFFFFF),
                          ),
                      ],
                    ),
                  ),
                ),
              );
            },
          );
        } else if (snapshot.hasError) {
          return Center(
            child: Container(
              color: Colors.white,
              child: dialogFail(context, reloadApp: true),
            ),
          );
        } else {
          return Center(
            child: Container(
              child: lv2 != '' ? const Text('Loading....') : Container(),
            ),
          );
        }
      },
    );
  }

  _buildLv4() {
    return FutureBuilder<dynamic>(
      future: futureLv4,
      builder: (BuildContext context, AsyncSnapshot<dynamic> snapshot) {
        if (snapshot.hasData) {
          return ListView.builder(
            shrinkWrap: true, // use it
            physics: const ClampingScrollPhysics(),
            itemCount: snapshot.data.length,
            itemBuilder: (context, index) {
              return InkWell(
                onTap: () {
                  setState(() {
                    lv4 = lv4 == snapshot.data[index]['code']
                        ? ''
                        : snapshot.data[index]['code'];
                  });
                },
                child: Container(
                  margin: const EdgeInsets.only(left: 50, right: 50, bottom: 5),
                  padding: const EdgeInsets.all(5),
                  decoration: BoxDecoration(
                      borderRadius: BorderRadius.circular(10),
                      color: lv4 == snapshot.data[index]['code']
                          ? const Color(0xFF359EEA)
                          : Colors.white,
                      border: Border.all(color: const Color(0xFF359EEA))),
                  alignment: Alignment.center,
                  child: Padding(
                    padding: const EdgeInsets.fromLTRB(5, 0, 5, 0),
                    child: Row(
                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
                      children: [
                        Text(
                          '${snapshot.data[index]['title']}',
                          style: TextStyle(
                            color: lv4 == snapshot.data[index]['code']
                                ? Colors.white
                                : Colors.black,
                          ),
                        ),
                        if (lv4 == snapshot.data[index]['code'])
                          const Icon(
                            Icons.check,
                            color: Color(0xFFFFFFFF),
                          ),
                      ],
                    ),
                  ),
                ),
              );
            },
          );
        } else if (snapshot.hasError) {
          return Center(
            child: Container(
              color: Colors.white,
              child: dialogFail(context, reloadApp: true),
            ),
          );
        } else {
          return Center(
            child: Container(
              child: lv3 != '' ? const Text('Loading....') : Container(),
            ),
          );
        }
      },
    );
  }

  _callSave() async {
    if (lv0 == '') {
      dialog(context,
          title: 'ไม่สามารถบันทึกข้อมูลได้',
          description: 'กรุณาเลือกประเภทการสื่อสาร \nอย่างน้อย 1 รายการ');
    } else {
      await postDio('${server}m/v2/register/organization/create', {
        'lv0': lv0,
        'lv1': lv1,
        'lv2': lv2,
        'lv3': lv3,
        'lv4': lv4,
      });

      Navigator.pop(context, true);
    }
  }

  _widgetText({required String title}) {
    return Text(
      title,
      textAlign: TextAlign.center,
      style: const TextStyle(
        fontWeight: FontWeight.bold,
        fontSize: 17,
      ),
    );
  }
}
